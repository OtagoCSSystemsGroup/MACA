...
class uav_swarm_env:
    def __init__(self):
        ...
        self.v2v_range =  self.obstacle_range * 2
    ...
    def reset_obs(self, x_cent=250, y_cent=150, vel_ang_init=180):
        ...
        for i in range(self.obs_num):
            ...
            if self.init_oind == 0:
                ...
                y_cent = obs_y_init[self.init_oind] + (-1) ** np.random.randint(2) * np.random.randint(50)
    ...
    def step(self, action, step, emergency_flag):
        ...
        # if np.mean(pos_obs[0]) - np.min(pos_swarm[0]) < -5:
        ...
        for i in range(self.agent_num):
            ...
            self.swarm[i].action(action[i][0], emergency_flag[i])
        ...
        collide_penalty = np.zeros((self.agent_num, 1))
        ...
        for i in range(self.agent_num):
                ...
                for obs in obs_ind:
                    ...
                    if observation_obs[0][0] == 2:
                        if not relative_position:
                            dist_temp = np.sqrt((observation_obs[1, 0]
                                                - state_[i][0, 0+np.int(self.id_concat), 1, 0]) ** 2
                                                + (observation_obs[1, 1]
                                                - state_[i][0, 0+np.int(self.id_concat), 1, 1]) ** 2)
                            vec_temp = [observation_obs[1, 0]
                                        - state_[i][0, 0+np.int(self.id_concat), 1, 0],
                                        observation_obs[1, 1]
                                        - state_[i][0, 0+np.int(self.id_concat), 1, 1]]
                        else:
            ...
            if len(ngh_ind) == 0:
                ...
                for ngh in ngh_ind:
                    if ngh == 0:
                        ...
                        if not relative_position:
                            dist_temp = np.sqrt((observation_ngh[1, 0]
                                                 - state_[i][0, 0+np.int(self.id_concat), 1, 0]) ** 2
                                                + (observation_ngh[1, 1]
                                                   - state_[i][0, 0+np.int(self.id_concat), 1, 1]) ** 2)
                            vec_temp = [observation_ngh[1, 0]
                                        - state_[i][0, 0+np.int(self.id_concat), 1, 0],
                                        observation_ngh[1, 1]
                                        - state_[i][0, 0+np.int(self.id_concat), 1, 1]]
                        else:
            ...
            if dist_after_action[i] <= 0:
                ...
                # collide_penalty[i] = np.tanh(dist_after_action[i]) * 10
                collide_penalty[i] = 0
            ...
            external_reward[i] = - collide_penalty[i]
        ...
        global_reward = np.sum(local_reward)
        # if any(collide):
        ...
        return state_, local_reward, global_reward, self_perception, done, info
    ...
    def get_image(self):
        ...
        for i in range(self.agent_num):
            ...
            if self.id_concat:
                img[0, :, 0] = id_onehot
            img[0+np.int(self.id_concat), :, 0] = [0, self.swarm[i].x,
                                                   self.swarm[i].vx, self.swarm[i].target_vx]
            img[0+np.int(self.id_concat), :, 1] = [0, self.swarm[i].y,
                                                   self.swarm[i].vy, self.swarm[i].target_vy]
                ...
                for j in range(self.agent_num):
                    if i != j:
                        ...
                        if dist <= self.swarm[i].observe_range:
                            near_ind += 1
                            if not relative_position:
                                img[near_ind, :, 0] = [1, self.swarm[j].x, self.swarm[j].vx, 0]
                                img[near_ind, :, 1] = [1, self.swarm[j].y, self.swarm[j].vy, 0]
                            else:
                                img[near_ind, :, 0] = [1, self.swarm[j].x - self.swarm[i].x,
                                                       self.swarm[j].vx, 0]
                                img[near_ind, :, 1] = [1, self.swarm[j].y - self.swarm[i].y,
                                                       self.swarm[j].vy, 0]
            ###################################################################################
            ...
            if self.obs_in_state:
                for k in range(self.obs_num):
                    ...
                    if dist <= self.swarm[i].observe_range or self.obstacle[k].detected:
                        ...
                        near_ind += 1
                        if not relative_position:
                            img[near_ind, :, 0] = [2, self.obstacle[k].x, self.obstacle[k].vx, 0]
                            img[near_ind, :, 1] = [2, self.obstacle[k].y, self.obstacle[k].vy, 0]
                        else:
                            img[near_ind, :, 0] = [2, self.obstacle[k].x - self.swarm[i].x,
                                                   self.obstacle[k].vx, 0]
                            img[near_ind, :, 1] = [2, self.obstacle[k].y - self.swarm[i].y,
                                                   self.obstacle[k].vy, 0]

            img[-1, 1, :] = [self.swarm[i].x_orig,
                             self.swarm[i].y_orig]
        ...
        for k in range(self.obs_num):
            img_global[round(self.obstacle[k].y)-5:round(self.obstacle[k].y)+5,
            round(self.obstacle[k].x - shift)-5:round(self.obstacle[k].x - shift)+5, 1] = 1  # Obstacle successive layers
