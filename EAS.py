...
def hyper_guarantee(env, action_pred, obs_x, obs_y, swarm_x, swarm_y, swarm_vel):
    action_cal = action_pred
    dist2obs_set = np.zeros([env.agent_num, 1])
    ...
    for agent in range(env.agent_num):
        ...
        action_noise = np.concatenate([np.linspace(-1, action_pred[agent], num=10, axis=1)[:, :-1],
                                       np.linspace(action_pred[agent], 1, num=10, axis=1)], axis=1)
        ...
        angle_map = emergency_angle_upper_bound - (env.action_upper_bound - action_noise) \
                    * (emergency_angle_upper_bound - emergency_angle_lower_bound) \
                    / (env.action_upper_bound - env.action_lower_bound)
        ...
        if len(collision_index) > 0:
            ...
            action_noise_prob[0][collision_index] = 0
            action_cal[agent] = action_noise[0, np.argmax(action_noise_prob)]
            angle_deg = angle_map[0, np.argmax(action_noise_prob)]
